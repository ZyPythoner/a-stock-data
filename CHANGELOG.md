...
本技能即数据验证铁律中的 **P2 第三源载体（腾讯财经 `qt.gtimg.cn`，HTTP 不封 IP、零鉴权）**。
新增「数据验证铁律 & 置信度标注」章节，含：

- **三源验证四步走流程**：① neodata-financial-search（P0 主源）→ ② TDX 通达信 / WebSearch / 东财·同花顺（P1 交叉验证）→ ③ 腾讯财经 `qt.gtimg.cn`（P2 第三源，本技能 `tencent_quote()`，A 股核心行情强制执行）→ ④ 标注置信度
- **置信度标注规范**：🔵三源（主源+独立源+腾讯一致）/ ✅双源 / ⚠️单源（必须显式提示）/ ❌未验证（如实标缺失，绝不虚构）
- **《数据验证声明》输出模板**：含数据报告末尾必须附，含数据源、验证状态、数据日期、版本号
- **硬约束**：A 股四大指数（上证/深成/创业板/科创50）不满足三源一致不得标注"已确认"；休市日腾讯源照常返回最后交易日数据，不得以"休市无法验证"跳过第三源

**四大指数三源验证标准用法**

基于 2026-08-28 盘中实测（上证 3952.47 / 深成 13953.00 / 创业板 3424.37 / 科创50 1662.54），
给出 `tencent_quote(["000001","399001","399006","000688"])` 批量拉取 + 三源比对（偏差 ≤0.02% 视为一致）+ 置信度标注的完整代码块。

### 修复

**安装路径环境校准**：原文档仅写 `~/.claude/skills/`（Claude Code 路径），
WorkBuddy 环境实际为 `~/.workbuddy/skills/`——「使用方式」与「安装说明」两处改为双路径兼容（WorkBuddy 为主、Claude Code 为备）。

**mootdx 服务器验证标注刷新**：2026-08-28 TCP 探测 10/10 存活，`_TDX_SERVERS` 无需更换，标注由"2026-06 验证"刷新为"2026-08-28 TCP 探测 10/10 存活"。
