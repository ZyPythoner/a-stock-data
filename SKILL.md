...
> **V3.7.2（数据验证铁律集成 + 环境校准，2026-08-28）：**在 V3.7.1 基础上合入以下增量——
> - **🔴 数据验证铁律集成（强制规则，DNA 级）**：本技能即数据验证铁律中的 **P2 第三源载体（腾讯财经 `qt.gtimg.cn`，HTTP 不封 IP、零鉴权）**。新增「数据验证铁律 & 置信度标注」章节：三源验证四步走流程（neodata 主源 → TDX/WebSearch 交叉 → 腾讯第三源 → 标注置信度）、置信度标注规范（🔵三源 / ✅双源 / ⚠️单源 / ❌未验证）、《数据验证声明》输出模板。A 股四大指数（上证/深成/创业板/科创50）不满足三源一致不得标注"已确认"；休市日腾讯源照常返回最后交易日数据，不得以此跳过第三源。
> - **📌 新增「四大指数三源验证」标准用法**（§1.2 后）：基于 2026-08-28 盘中实测（上证 3952.47 / 深成 13953.00 / 创业板 3424.37 / 科创50 1662.54），给出 `tencent_quote()` 批量拉取四大指数 + 三源比对（偏差 ≤0.02% 视为一致）+ 置信度标注的完整代码块。
> - **🔧 安装路径环境校准**：原文档仅写 `~/.claude/skills/`（Claude Code 路径），WorkBuddy 环境实际为 `~/.workbuddy/skills/`——「使用方式」与「安装说明」两处改为双路径兼容（WorkBuddy 为主、Claude Code 为备）。
> - **✅ mootdx 服务器 10/10 实测存活（2026-08-28 TCP 探测）**：`_TDX_SERVERS` 全部连通，验证标注由"2026-06"刷新为"2026-08-28"。
>
...
**使用方式：** 将本文件放入 `~/.workbuddy/skills/a-stock-data/SKILL.md`（WorkBuddy）或 `~/.claude/skills/a-stock-data/SKILL.md`（Claude Code），WorkBuddy / Claude Code 会自动识别并在 A 股相关对话中激活。
...
## 数据验证铁律 & 置信度标注（V3.7.2 强制规则，先读）

> **本技能是数据验证铁律的 P2 第三源载体**：腾讯财经 `qt.gtimg.cn`（HTTP 不封 IP、零鉴权）。涉及 A 股行情 / 财务数据 / 技术指标 / 任何含具体数字的结论时，必须执行下方交叉验证流程，不得跳过。

### 三源验证流程（四步走）

| 步骤 | 角色 | 工具 | 说明 |
|------|------|------|------|
| ① 主数据源 | P0 主源 | neodata-financial-search | 金融搜索引擎取数 |
| ② 交叉验证 | P1 验证源 | TDX 通达信（行情）/ WebSearch（新闻事件）/ 东财·同花顺（板块·个股） | 独立源核对 |
| ③ 第三源补充 | **P2 第三源（本技能）** | 腾讯财经 `qt.gtimg.cn`（`tencent_quote()`） | A 股核心行情**强制执行**，HTTP 不封 IP |
| ④ 标注置信度 | 输出规范 | 见下方标注规则 | 每条关键数据必须标注 |

> ⛔ **A 股四大指数（上证 / 深成 / 创业板 / 科创50）必须三源一致才能标注"已确认"**；未经双源确认的数据必须标注"单源，请注意"。
>
> ⛔ **休市日 ≠ 跳过第三源**：腾讯财经 `qt.gtimg.cn` 在休市日同样返回最后交易日数据，不得以"休市无法验证"为由跳过 P2 第三源。

### 置信度标注规则（输出数据时强制）

| 标注 | 含义 | 要求 |
|------|------|------|
| 🔵 三源 | 主源 + 独立源 + 腾讯第三源一致 | 四大指数、核心行情、关键结论 |
| ✅ 双源 | 两个独立源一致 | 一般行情/财务数据 |
| ⚠️ 单源 | 仅一个数据源 | 必须显式提示"单源，请注意" |
| ❌ 未验证 | 无接口数据 | 如实标注"缺失"，绝不虚构 |

### 输出规范

1. 含数据的报告，末尾必须附 **《数据验证声明》** 章节，模板：

```markdown
## 数据验证声明
- 数据源：腾讯财经 qt.gtimg.cn / mootdx / 东财（注明具体端点）
- 验证状态：上证指数 🔵三源一致 / XXX ✅双源 / XXX ⚠️单源
- 数据日期：YYYY-MM-DD（休市日注明"最后交易日数据"）
- 版本：A股全栈数据 V3.7.2
```

2. 股票代码禁止凭记忆编写，必须先过 `norm_ticker()` 校验 + 双源交叉（westock + 腾讯财经 `qt.gtimg.cn`）。
3. 发现有误立即修正，并在报告中附更正记录。
4. 分析结论层（个股买卖决策）由调用方加载 `guiding-spirit`（五原则检验）与 `professional-trader`（操作型决策），本技能只负责数据层准确性，不越权出投资结论。

### 四大指数三源验证 · 标准用法（V3.7.2 新增）

```python
# A股四大指数（腾讯源 = P2 第三源）：
#   sh000001=上证指数  sz399001=深证成指  sz399006=创业板指  sh000688=科创50
# 实测 2026-08-28 盘中：上证 3952.47 / 深成 13953.00 / 创业板 3424.37 / 科创50 1662.54
index_quotes = tencent_quote(["000001", "399001", "399006", "000688"])
for code, q in index_quotes.items():
    print(f"{q['name']}: {q['price']} 涨跌幅{q['change_pct']}%")

# 三源验证核对流程（每源独立拉取后比对价格，偏差 ≤ 0.02% 视为一致）：
#   ① neodata-financial-search 拉指数（P0 主源）
#   ② TDX 通达信 / westock 拉指数（P1 交叉）
#   ③ 上方 tencent_quote() 结果（P2 第三源，本技能）
#   ④ 三方价格一致 → 标注 🔵三源一致；仅两方 → ✅双源；单方 → ⚠️单源，请注意
```
...
# 实测可用的备选服务器（按延迟排序，2026-08-28 TCP 探测 10/10 存活）
_TDX_SERVERS = [
...
```bash
# 1. 创建 skill 目录（WorkBuddy 环境；Claude Code 则用 ~/.claude/skills/a-stock-data）
mkdir -p ~/.workbuddy/skills/a-stock-data

# 2. 将本文件复制为 SKILL.md
cp SKILL.md ~/.workbuddy/skills/a-stock-data/SKILL.md
...
# 5. 启动 WorkBuddy / Claude Code，说"查一下688017的估值"即可自动激活
```
